locals {
  ...
  result_envs_connections = [for name, value in local.mapped_envs_connections : { name = name, value = jsondecode(data.jq_query.envs_connections["${name}"].result) }]
  ...
  result_envs_params = [for name, value in local.mapped_envs_params : { name = name, value = jsondecode(data.jq_query.envs_params["${name}"].result) }]
